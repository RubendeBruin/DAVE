...
def generate_python_code(cls, imports = True):
    ...
    if imports:
        ...
        code.append('from DAVE.settings import NodePropertyInfo, DAVE_ADDITIONAL_RUNTIME_MODULES, DAVE_NODEPROP_INFO')

    cls_name = cls.__name__


    code.append(f'# ===================== Auto-generated documentation registration for {cls_name}')
    ...
    for name, what in cls.__dict__.items():
        ...
        if type(what) == property:
            ...
            code.append(f'                        is_single_settable = {is_single_settable},')
            code.append(f'                        is_single_numeric = {is_single_numeric}')
            code.append(f'                        )')
