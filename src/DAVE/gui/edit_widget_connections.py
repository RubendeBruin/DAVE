...
class EditConnections(NodeEditor):
    def __init__(self):
        ...
        # support for SlingGrommet (needed???)
        self.endAFr = False
        ...
        self.ui.tree.setHeaderLabels(
            ["⭮/⭯", "Connection", "Offset", "Max winding", "Use Friction", "Force Factor", "Cable point",
             "Circle point"]
        )
    ...
    def post_update_event(self):
        ...
        self.ui.pbSetShortestRoute.setVisible(hasattr(self.node, 'set_optimum_connection_directions'))
    ...
    def _update_model_from_node(self):
        ...
        a_friction_type = self.node._make_connection_aligned_copy_of_friction_vector(self.node.friction_type,
                                                                                     insert_value=FrictionType.No)
        a_friction_factor = self.node._make_connection_aligned_copy_of_friction_vector(self.node.friction,
                                                                                       insert_value=None)
        a_friction_cable_point = self.node._make_connection_aligned_copy_of_friction_vector(
            self.node.friction_point_cable, insert_value=None)
        a_friction_connector_point = self.node._make_connection_aligned_copy_of_friction_vector(
            self.node.friction_point_connection, insert_value=None)

        self.connections_model.clear()

        for connection, reversed, offs, friction_type, friction_factor, friction_cable_point, friction_connector_point, max_winding in zip(
            ...
            self.connections_model.append(
                CableConnection(connection, reversed, offs, max_winding, friction_type, friction_factor,
                                friction_cable_point, friction_connector_point))
    ...
    def _update_ui_from_model(self):
        ...
        is_loop = self.connections_model[0].connection_node == self.connections_model[-1].connection_node
        ...
        for i, connection in enumerate(self.connections_model):
            ...
            item.setToolTip(1, connection.connection_node.name)  # store the node name as tooltop
            ...
            if isinstance(connection.connection_node, (Circle)):
                item.setCheckState(0, Qt.CheckState.Checked if connection.reversed else Qt.CheckState.Unchecked)


            # 4 : friction type
            if i == 0 and not is_loop or i == N - 1:
    ...
    def _update_model_from_ui(self):
        ...
        for i in range(self.ui.tree.topLevelItemCount()):
            ...
            self.connections_model.append(
                CableConnection(connection, reversed, offset, max_winding, friction_type, friction_factor,
                                friction_cable_point, friction_connector_point))

    def _update_node_from_model(self):
        ...
        # friction vectors
        cons = self.connections_model[:-1]  # no friction info for the last connection
        if not is_loop:
        ...
        # check the values
        self.node: Cable  # type hint only
        errors = self.node._check_friction_vectors(connections=connection_names,
        ...
        element = f"\ns['{self.node.name}']"
        code += f"{element}.update_connections(connections=connections,\n" \
                "                             reversed=reversed,\n" \
                "                             offsets=offsets,\n" \
                "                             max_winding_angles=max_winding_angles,\n" \
    ...
    def add_item(self):
        ...
        if self.scene.node_exists(name):
            ...
            self.connections_model.append(CableConnection(self.scene[name], False, 0, 0, FrictionType.No, None, None,
                                                          None))
    ...
    def dropEvent(self, event):
        ...
        if event.source() == tree:  # internal move
            ...
            if to_row >= 0:
                self.connections_model.insert(to_row, self.connections_model.pop(from_row))
            else:
            ...
            if event.mimeData().hasText():
                ...
                new_connection = CableConnection(node, False, 0, 0, FrictionType.No, None, None, None)
                if to_row > 0:
