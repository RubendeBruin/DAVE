...
def rotmat_to_az_dec_gam(orientation):
    Rx = np.arctan2(
        +orientation[2][1], +orientation[2][0]
    )  # = azimuth(+orientation[2]) [0,2*pi]
    ...
    if abs(cosDeclination) < 1.0 / 2.0**0.5:
        ...
        Rz = np.arctan2(
            -(cosAzimuth * orientation[1][0] + sinAzimuth * orientation[1][1])
            / cosDeclination,
            +(cosAzimuth * orientation[0][0] + sinAzimuth * orientation[0][1])
            / cosDeclination,
        )  # [-pi,pi]
    ...
    for n in s._nodes:
        ...
        if isinstance(n, (RigidBody, Frame)):
            ...
            if hyd_spring is not None:
                ...
                k[0, 1] = -hyd_spring.kHeave * (
                    hyd_spring.cob[1] + hyd_spring.COFY - ref_origin[1]
                )  # heave-roll
                k[0, 2] = -hyd_spring.kHeave * (
                    hyd_spring.cob[0] + hyd_spring.COFX - ref_origin[0]
                )  # heave-pitch
                k[1, 0] = k[0, 1]
                k[2, 0] = k[0, 2]

                # BML and BMT
                k[1, 1] = hyd_spring.displacement_kN * (
                    hyd_spring.BMT - ref_origin[2] + hyd_spring.cob[2]
                )  # g * rho * disp * BMt
                k[2, 2] = hyd_spring.displacement_kN * (
                    hyd_spring.BML - ref_origin[2] + hyd_spring.cob[2]
                )  # g * rho * disp * BMt
                ...
                d = {
                    ...
                    "HydrostaticReferenceOriginDatumPositionz": n.to_glob_position(
                        ref_origin
                    )[2],
                ...
                if hyd_db is not None:
                    ...
                    LoadRAOs = {
                        "RAOOriginX": ref_origin[
                            0
                        ],  # TODO: These values do not seem to be loaded into OFX
                    ...
                    db = Hyddb1.create_from(database)
        ...
        if isinstance(n, Visual):
            ...
            if "obj" in visualfile.suffix:
                ...
                warn(
                    f'Only .obj files can be used in orcaflex, not exporting visual "{n.name}"'
                )
